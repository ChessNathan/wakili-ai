...
/**
 * Uses Gemini to extract client name, document title, and case/cause number
 * from the raw text of an imported legal document.
 */
async function extractDocumentMetadata(text: string, fileName: string): Promise<{
  ...
  const snippet = text.slice(0, 4000); // Use first 4k chars — enough for a header/preamble
  const prompt = `You are a Kenyan legal document parser. Extract the following from this document text and return ONLY valid JSON, no markdown, no explanation:
- "document_title": The formal title of the document (e.g. "Plaint", "Sale Agreement", "Demand Letter"). If not found, use the file name: "${fileName}".
- "client_name": The primary client or plaintiff's full name. If not found, return null.
- "case_number": Any court cause/case number (e.g. "HCCC No. 45 of 2024", "ELC No. 12/2023"). If not found, return null.
...
Return only this JSON shape: {"document_title": "...", "client_name": "...", "case_number": "..."}`;
  ...
  try {
    ...
    logger.warn('Metadata extraction failed, using defaults', { error: err.message });
...
googleRouter.post('/import-from-drive', async (req: AuthRequest, res: Response): Promise<void> => {
  ...
  try {
    ...
    if (fileMime === 'application/vnd.google-apps.document') {
      ...
      // ── PDF / Word: export as plain text via Drive export ───
      const exportMime = 'text/plain';
      const exported   = await driveApi.files.export(
        { fileId: google_file_id, mimeType: exportMime },
        { responseType: 'text' },
      );
      text = (exported.data as string) || '';

    } else {
      // ── Unknown type: try plain-text export ─────────────────
      try {
        const exported = await driveApi.files.export(
