...
export const api = {
  ...
  ai: {
    ...
    analyseDocument: (document_id: string) =>
      req<{ analysis: {
        summary: string; key_issues: string[]; legal_risks: string[];
        recommended_actions: string[]; case_strength: string; relevant_laws: string[];
      } }>('/api/ai/analyse', { method: 'POST', body: JSON.stringify({ document_id }) }),
  },
  documents: {
  ...
  google: {
    ...
    driveFiles:       (q?: string, pageToken?: string) =>
      req<{ files: DriveFile[]; nextPageToken: string | null }>(`/api/google/drive-files${q ? `?q=${encodeURIComponent(q)}` : ''}${pageToken ? `${q ? '&' : '?'}pageToken=${pageToken}` : ''}`),
    importFromDrive:  (google_file_id: string, title?: string, mimeType?: string) =>
      req<{ document: Document; extracted_metadata: { client_name: string | null; document_title: string | null; case_number: string | null } }>('/api/google/import-from-drive', { method: 'POST', body: JSON.stringify({ google_file_id, title, mimeType }) }),
  },
};
