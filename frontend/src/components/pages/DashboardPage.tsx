...
import { PageHeader, DocTypeBadge, Spinner } from '../ui/index';
...
export function DashboardPage() {
  ...
  const thisWeekDocs = docs.filter(d => new Date(d.created_at) > new Date(Date.now() - 7 * 864e5)).length;
    ...
    <div style={{ display: 'flex', flexDirection: 'column', height: '100%', overflow: 'hidden' }}>
      ...
      <div style={{ flex: 1, overflowY: 'auto', padding: '22px 28px', display: 'flex', flexDirection: 'column', gap: 24 }}>
        ...
        <div style={{ background: 'linear-gradient(135deg, var(--emerald) 0%, var(--emerald-mid) 100%)', borderRadius: 16, padding: '24px 28px', position: 'relative', overflow: 'hidden' }}>
          ...
          <div style={{ position: 'absolute', right: 24, top: '50%', transform: 'translateY(-50%)', fontFamily: 'var(--font-display)', fontSize: 80, fontWeight: 700, color: 'rgba(201,168,76,0.12)', pointerEvents: 'none', userSelect: 'none', lineHeight: 1 }}>W</div>
          <div style={{ position: 'relative' }}>
            <div style={{ fontFamily: 'var(--font-display)', fontSize: 22, fontWeight: 700, color: '#fff', marginBottom: 4 }}>
              {greeting}, {profile?.full_name?.split(' ')[0]} ⚖️
            </div>
            <div style={{ fontSize: 13, color: 'rgba(255,255,255,0.65)' }}>
              {activeCases} active case{activeCases !== 1 ? 's' : ''} · {draftDocs} draft{draftDocs !== 1 ? 's' : ''} in progress
              {urgentDeadlines > 0 && <span style={{ color: '#fca5a5', marginLeft: 8 }}>· ⚠️ {urgentDeadlines} urgent deadline{urgentDeadlines !== 1 ? 's' : ''}</span>}
            </div>
          </div>
        </div>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: 12 }}>
          {[
            { label: 'Active Cases',    value: activeCases,    icon: '⚖️',  color: 'var(--emerald)',     path: '/cases' },
            { label: 'Total Documents', value: docs.length,    icon: '📄',  color: 'var(--gold)',        path: '/documents' },
            { label: 'This Week',       value: thisWeekDocs,   icon: '✍️',  color: '#6d28d9',            path: '/drafter' },
            { label: 'Deadlines',       value: deadlines.length, icon: '📅', color: urgentDeadlines > 0 ? 'var(--red)' : 'var(--emerald-light)', path: '/deadlines' },
          ].map(s => (
            ...
            >
              <div style={{ position: 'absolute', bottom: 0, left: 0, right: 0, height: 3, background: s.color, opacity: 0.6 }} />
        ...
        <div>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: 10 }}>
            {[
              { icon: '✍️', label: 'Draft a Pleading',     sub: 'AI-powered court document',   path: '/drafter', color: 'var(--emerald)' },
              { icon: '📋', label: 'Draft a Contract',     sub: 'Agreements & SLAs',            path: '/drafter', color: '#6d28d9' },
              { icon: '📬', label: 'Draft Demand Letter',  sub: 'Formal legal demand',          path: '/drafter', color: 'var(--red)' },
              { icon: '⚖️', label: 'New Case',             sub: 'Open a matter',                path: '/cases',   color: 'var(--gold)' },
              { icon: '👥', label: 'Add Client',           sub: 'Register a new client',        path: '/clients', color: '#0284c7' },
              { icon: '📅', label: 'Set Deadline',         sub: 'Track a filing date',          path: '/deadlines', color: '#059669' },
            ].map(a => (
              ...
              >
                <div style={{ width: 36, height: 36, borderRadius: 9, background: `${a.color}18`, display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 18, flexShrink: 0 }}>{a.icon}</div>
                <div>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 18 }}>
          ...
          <div>
            <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 12 }}>
              ...
              <button onClick={() => navigate('/documents')} style={{ fontSize: 11, color: 'var(--gold)', background: 'none', border: 'none', cursor: 'pointer', textDecoration: 'underline', textUnderlineOffset: 2 }}>See all →</button>
            </div>
            <div style={{ display: 'flex', flexDirection: 'column', gap: 7 }}>
              {docs.slice(0, 5).length === 0 ? (
                ...
                >
                  ...
                  {d.google_doc_id && <span title="Linked to Google Docs" style={{ fontSize: 14 }}>📝</span>}
                </div>
              ))}
            </div>
          </div>
          ...
          <div>
            <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 12 }}>
              ...
              <button onClick={() => navigate('/deadlines')} style={{ fontSize: 11, color: 'var(--gold)', background: 'none', border: 'none', cursor: 'pointer', textDecoration: 'underline', textUnderlineOffset: 2 }}>See all →</button>
            </div>
            <div style={{ display: 'flex', flexDirection: 'column', gap: 7 }}>
              {deadlines.slice(0, 5).length === 0 ? (
                ...
                const colors = { urgent: { dot: '#b84040', bg: '#fde8e8' }, soon: { dot: '#c9a84c', bg: '#fef9ee' }, normal: { dot: '#4ade80', bg: '#f0fdf4' } };
                const c = colors[d.urgency] || colors.normal;
                ...
                return (
                  ...
                  >
                    ...
                    <span style={{ fontSize: 10, fontWeight: 700, padding: '2px 8px', borderRadius: 8, background: c.bg, color: c.dot, flexShrink: 0 }}>{label}</span>
                  </div>
                );
              })}
            </div>
