...
import { Settings, User, Building2, Globe, CheckCircle, XCircle, ExternalLink } from 'lucide-react';
...
export function SettingsPage() {
  ...
  useEffect(() => {
    ...
    else if (p.get('google')==='error') showToast('Google connection failed: '+p.get('reason'));
  }, []);
  ...
  async function disconnectGoogle() {
    ...
    try {
      ...
      setGoogleConnected(false); showToast('Google account disconnected.');
    } catch(e:any) { showToast('Error: '+e.message); }
  }
  ...
  return (
    <div style={{display:'flex',flexDirection:'column',height:'100%',overflow:'hidden'}}>
      ...
      {toast&&(
        <div style={{position:'fixed',top:20,right:20,background:'var(--forest)',color:'#fff',borderRadius:10,padding:'12px 20px',fontSize:14,boxShadow:'var(--shadow-lg)',zIndex:2000,animation:'fadeUp 0.2s ease'}}>{toast}</div>
      )}
      ...
      <div style={{flex:1,overflowY:'auto',padding:'28px'}}>
        ...
        {tab==='integrations'&&(
          <div style={{maxWidth:680,display:'flex',flexDirection:'column',gap:20}}>
              ...
              <div style={{padding:'20px 24px',borderBottom:'1px solid var(--border)',display:'flex',alignItems:'center',gap:14}}>
                ...
                {checkingGoogle ? <Spinner size={20}/> : (
                  <div style={{display:'flex',alignItems:'center',gap:6,padding:'6px 14px',borderRadius:20,background:googleConnected?'var(--green-dim)':'#F3F4F6',border:`1px solid ${googleConnected?'rgba(22,163,74,0.2)':'var(--border)'}`}}>
                    {googleConnected ? <CheckCircle size={14} color="var(--green)"/> : <XCircle size={14} color="var(--muted)"/>}
                    <span style={{fontSize:13,fontWeight:600,color:googleConnected?'var(--green)':'var(--muted)'}}>{googleConnected?'Connected':'Not connected'}</span>
                  </div>
                )}
              </div>
                ...
                {!googleConnected ? (
                  <div style={{display:'flex',flexDirection:'column',gap:16}}>
                    <div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:10}}>
                      {[{t:'Save to Drive',b:'Documents auto-saved to Wakili AI folder'},{t:'Edit in Google Docs',b:'Full Google Docs editing experience'},{t:'Sync Changes',b:'Pull edits back into Wakili AI'},{t:'Team Sharing',b:'Share via Google Drive permissions'}].map(f=>(
                    ...
                    <Button variant="primary" onClick={connectGoogle} leftIcon={<ExternalLink size={15}/>} style={{alignSelf:'flex-start'}}>Connect Google Account</Button>
                  </div>
                ) : (
            ...
            <div style={{background:'#FFFBEB',border:'1px solid #FCD34D',borderRadius:14,padding:'20px 24px'}}>
              <div style={{fontSize:15,fontWeight:700,color:'#92400E',marginBottom:14,display:'flex',alignItems:'center',gap:8}}><Settings size={16}/>Google Cloud Console Setup</div>
              <ol style={{fontSize:14,color:'var(--ink)',lineHeight:2.2,paddingLeft:20}}>
                ...
                <li>Redirect URI: <code style={{background:'#FEF3C7',padding:'2px 8px',borderRadius:5,fontSize:13}}>https://your-backend.onrender.com/api/google/callback</code></li>
        ...
        {tab==='firm'&&(
          <div style={{maxWidth:540}}>
            <div style={{background:'#fff',border:'1px solid var(--border)',borderRadius:14,overflow:'hidden',boxShadow:'var(--shadow-sm)'}}>
              <div style={{padding:'18px 24px',borderBottom:'1px solid var(--border)',background:'var(--bg)'}}><div style={{fontSize:17,fontWeight:600,color:'var(--ink)'}}>Firm Details</div></div>
              <div style={{padding:'22px 24px',display:'flex',flexDirection:'column',gap:16}}>
                {[{l:'Firm Name',v:firm?.name||'—'},{l:'Plan',v:firm?`${firm.plan.charAt(0).toUpperCase()+firm.plan.slice(1)} Plan`:'—'},{l:'Firm ID',v:firm?.id||'—',mono:true},{l:'Created',v:firm?new Date(firm.created_at).toLocaleDateString('en-KE',{day:'numeric',month:'long',year:'numeric'}):'—'}].map(r=>(
                  <div key={r.l} style={{paddingBottom:14,borderBottom:'1px solid var(--border)'}}>
                    <div style={{fontSize:13,fontWeight:500,color:'var(--muted)',marginBottom:4,textTransform:'uppercase',letterSpacing:'0.06em'}}>{r.l}</div>
                    <div style={{fontSize:16,color:'var(--ink)',fontFamily:r.mono?'monospace':'var(--font-body)',wordBreak:'break-all'}}>{r.v}</div>
                  </div>
                ))}
              </div>
            </div>
        ...
        {tab==='account'&&(
          <div style={{maxWidth:540}}>
            <div style={{background:'#fff',border:'1px solid var(--border)',borderRadius:14,overflow:'hidden',boxShadow:'var(--shadow-sm)'}}>
              <div style={{padding:'18px 24px',borderBottom:'1px solid var(--border)',background:'var(--bg)'}}><div style={{fontSize:17,fontWeight:600,color:'var(--ink)'}}>My Account</div></div>
              <div style={{padding:'22px 24px'}}>
                <div style={{display:'flex',alignItems:'center',gap:16,marginBottom:22}}>
                  <div style={{width:56,height:56,borderRadius:'50%',background:'linear-gradient(135deg,var(--gold),var(--forest-light))',display:'flex',alignItems:'center',justifyContent:'center',fontSize:20,fontWeight:700,color:'#fff',flexShrink:0}}>{profile?.initials||'?'}</div>
                  <div>
                    <div style={{fontSize:20,fontWeight:700,color:'var(--ink)'}}>{profile?.full_name}</div>
                    <div style={{fontSize:14,color:'var(--muted)',marginTop:3,textTransform:'capitalize'}}>{profile?.role?.replace(/_/g,' ')}</div>
                  </div>
                </div>
                <div style={{display:'flex',flexDirection:'column',gap:14}}>
                  {[{l:'Role',v:profile?.role?.replace(/_/g,' ')||'—'},{l:'User ID',v:profile?.id||'—',mono:true},{l:'Member Since',v:profile?new Date(profile.created_at).toLocaleDateString('en-KE',{day:'numeric',month:'long',year:'numeric'}):'—'}].map(r=>(
                    <div key={r.l} style={{paddingBottom:12,borderBottom:'1px solid var(--border)'}}>
                      <div style={{fontSize:13,fontWeight:500,color:'var(--muted)',marginBottom:4,textTransform:'uppercase',letterSpacing:'0.06em'}}>{r.l}</div>
                      <div style={{fontSize:15,color:'var(--ink)',fontFamily:r.mono?'monospace':'var(--font-body)',wordBreak:'break-all',textTransform:'capitalize'}}>{r.v}</div>
                    </div>
                  ))}
                </div>
              </div>
