...
import { PageHeader, Button, DocTypeBadge, EmptyState, Spinner, Modal } from '../ui/index';
...
export function DocumentsPage() {
  ...
  useEffect(() => { if (profile?.firm_id) fetchDocs(); }, [profile?.firm_id, filterType, filterStatus]);
  ...
  async function handleStatusChange(id: string, status: string) {
    const { data } = await supabase.from('documents').update({ status }).eq('id', id).select().single();
    if (data) setDocs(prev => prev.map(d => d.id === id ? { ...d, status: status as DocStatus } : d));
  }

  return (
    <div style={{ display: 'flex', flexDirection: 'column', height: '100%', overflow: 'hidden' }}>
      <PageHeader title="Documents" subtitle={`${docs.length} document${docs.length !== 1 ? 's' : ''} in your firm`} />
      ...
      <div style={{ flex: 1, overflowY: 'auto', padding: '18px 24px' }}>
        {loading ? (
          ...
          <div style={{ background: '#fff', border: '1px solid var(--border)', borderRadius: 12, overflow: 'hidden' }}>
            <div style={{ display: 'grid', gridTemplateColumns: '2fr 1fr 1fr 1fr 1fr auto', borderBottom: '1px solid var(--border)', background: 'var(--cream)' }}>
            ...
            {filtered.map((d, i) => (
              <div key={d.id}
                style={{ display: 'grid', gridTemplateColumns: '2fr 1fr 1fr 1fr 1fr auto', borderBottom: i < filtered.length - 1 ? '1px solid var(--border)' : 'none', transition: 'background 0.12s', cursor: 'pointer' }}
              ...
              >
                ...
                <div style={{ padding: '13px 14px', display: 'flex', alignItems: 'center' }}>
                  <select value={d.status} onChange={e => { e.stopPropagation(); handleStatusChange(d.id, e.target.value); }}
                ...
                <div style={{ padding: '13px 14px', display: 'flex', alignItems: 'center' }}>
                  <span style={{ fontSize: 12, color: 'var(--muted)' }}>{(d as any).cases?.ref_number || '—'}</span>
                </div>
                <div style={{ padding: '13px 14px', display: 'flex', alignItems: 'center' }}>
                  <span style={{ fontSize: 12, color: 'var(--muted)' }}>{(d as any).profiles?.initials || '—'}</span>
                </div>
