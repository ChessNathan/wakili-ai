...
// ── Drive Import Modal ───────────────────────────────────────
function DriveImportModal({ onClose, onImport }: { onClose: () => void; onImport: (doc: Document, metadata?: { client_name: string | null; document_title: string | null; case_number: string | null }) => void }) {
  ...
  async function handleImport(file: DriveFile) {
    ...
    try {
      ...
      onImport(res.document, res.extracted_metadata);
      onClose();
...
export function DrafterPage() {
  ...
  const [importLinkModal, setImportLinkModal] = useState<{
    doc: Document;
    clientName: string;
    docTitle: string;
    caseNumber: string;
  ...
  function handleImported(imported: Document, metadata?: { client_name: string | null; document_title: string | null; case_number: string | null }) {
    setDoc(imported);
    setRecent(prev => [imported, ...prev.slice(0, 11)]);
    setView('preview');
    // Open the link modal so the user can confirm auto-detected info
    setImportLinkModal({
      doc: imported,
      clientName:     metadata?.client_name    ?? '',
      docTitle:       metadata?.document_title ?? imported.title,
  ...
  async function saveImportLink() {
    if (!importLinkModal) return;
    setImportLinkModal(m => m ? { ...m, saving: true } : null);
    try {
      const updates: Record<string, any> = {
        title:      importLinkModal.docTitle || importLinkModal.doc.title,
        updated_at: new Date().toISOString(),
      };
      if (importLinkModal.selectedCaseId) updates.case_id = importLinkModal.selectedCaseId;
      // Store client_name and case_number in notes if not yet linked to a case
      const extra: string[] = [];
      if (importLinkModal.clientName)  extra.push(`Client: ${importLinkModal.clientName}`);
      if (importLinkModal.caseNumber)  extra.push(`Case No: ${importLinkModal.caseNumber}`);
      if (extra.length && !importLinkModal.selectedCaseId) {
        const existing = importLinkModal.doc.content || '';
        // Prepend metadata header to content for reference
        updates.content = `[${extra.join(' | ')}]\n\n${existing}`;
      }
      const { data: updated } = await supabase.from('documents').update(updates).eq('id', importLinkModal.doc.id).select('*, cases(title,ref_number)').single();
      if (updated) {
        setDoc(updated as Document);
        setRecent(prev => prev.map(d => d.id === updated.id ? updated as Document : d));
      }
      setImportLinkModal(null);
    } catch {
      setImportLinkModal(m => m ? { ...m, saving: false } : null);
    }
  }
  ...
  return (
    <>
      ...
      {/* ── Import Link Modal: confirm AI-detected metadata ── */}
      {importLinkModal && (
        <div style={{ position:'fixed', inset:0, background:'rgba(0,0,0,0.55)', zIndex:1100, display:'flex', alignItems:'center', justifyContent:'center', padding:24 }}>
          <div style={{ background:'#fff', borderRadius:18, width:'100%', maxWidth:520, boxShadow:'0 28px 80px rgba(0,0,0,0.22)', overflow:'hidden' }}>
            {/* Header */}
            <div style={{ padding:'20px 24px 16px', borderBottom:'1px solid var(--border)', display:'flex', alignItems:'center', gap:12 }}>
              ...
              <div>
                <div style={{ fontSize:17, fontWeight:700, color:'var(--ink)' }}>Document Detected</div>
                <div style={{ fontSize:13, color:'var(--muted)', marginTop:2 }}>AI has read your document — confirm or edit details before saving</div>
              </div>
            </div>
            ...
            <div style={{ padding:'20px 24px', display:'flex', flexDirection:'column', gap:14 }}>
              {/* Document Title */}
              <div>
                <label style={{ fontSize:12, fontWeight:600, color:'var(--muted)', textTransform:'uppercase', letterSpacing:'0.07em', display:'block', marginBottom:5 }}>Document Title</label>
                <input
                  value={importLinkModal.docTitle}
                  onChange={e => setImportLinkModal(m => m ? { ...m, docTitle: e.target.value } : null)}
                  style={{ width:'100%', border:'1.5px solid var(--border)', borderRadius:8, padding:'9px 13px', fontSize:14, fontFamily:'var(--font-body)', outline:'none', boxSizing:'border-box' }}
              ...
              {/* Client Name */}
              <div>
                <label style={{ fontSize:12, fontWeight:600, color:'var(--muted)', textTransform:'uppercase', letterSpacing:'0.07em', display:'block', marginBottom:5 }}>
                  Client Name
                  {importLinkModal.clientName
                    ? <span style={{ marginLeft:8, fontWeight:400, textTransform:'none', letterSpacing:0, color:'var(--forest)', fontSize:11 }}>✓ auto-detected</span>
                    : <span style={{ marginLeft:8, fontWeight:400, textTransform:'none', letterSpacing:0, color:'var(--muted)', fontSize:11 }}>not found in document</span>}
                </label>
                <input
                  value={importLinkModal.clientName}
                  onChange={e => setImportLinkModal(m => m ? { ...m, clientName: e.target.value } : null)}
              ...
              <div>
                <label style={{ fontSize:12, fontWeight:600, color:'var(--muted)', textTransform:'uppercase', letterSpacing:'0.07em', display:'block', marginBottom:5 }}>
                  Case / Cause Number
                  {importLinkModal.caseNumber
                    ? <span style={{ marginLeft:8, fontWeight:400, textTransform:'none', letterSpacing:0, color:'var(--forest)', fontSize:11 }}>✓ auto-detected</span>
                    : <span style={{ marginLeft:8, fontWeight:400, textTransform:'none', letterSpacing:0, color:'var(--muted)', fontSize:11 }}>not found in document</span>}
                </label>
                <input
                  value={importLinkModal.caseNumber}
                  onChange={e => setImportLinkModal(m => m ? { ...m, caseNumber: e.target.value } : null)}
                  placeholder="e.g. HCCC No. 45 of 2024"
                  style={{ width:'100%', border:'1.5px solid var(--border)', borderRadius:8, padding:'9px 13px', fontSize:14, fontFamily:'var(--font-body)', outline:'none', boxSizing:'border-box' }}
                  onFocus={e => (e.target.style.borderColor = 'var(--forest)')}
                  onBlur={e => (e.target.style.borderColor = 'var(--border)')}
                />
              ...
              <div>
                <label style={{ fontSize:12, fontWeight:600, color:'var(--muted)', textTransform:'uppercase', letterSpacing:'0.07em', display:'block', marginBottom:5 }}>Link to Existing Case <span style={{ fontWeight:400, textTransform:'none', letterSpacing:0, fontSize:11 }}>(optional)</span></label>
                <select
                  value={importLinkModal.selectedCaseId}
                  onChange={e => setImportLinkModal(m => m ? { ...m, selectedCaseId: e.target.value } : null)}
                  style={{ width:'100%', border:'1.5px solid var(--border)', borderRadius:8, padding:'9px 13px', fontSize:14, fontFamily:'var(--font-body)', color:'var(--ink)', background:'#fff', outline:'none', boxSizing:'border-box', cursor:'pointer' }}
                  onFocus={e => (e.target.style.borderColor = 'var(--forest)')}
                  onBlur={e => (e.target.style.borderColor = 'var(--border)')}
                >
                  <option value="">— No case link —</option>
                  {cases.map(c => (
                    <option key={c.id} value={c.id}>{c.ref_number} · {c.title}</option>
                  ))}
                </select>
              </div>
            </div>
            ...
            <div style={{ padding:'14px 24px 20px', borderTop:'1px solid var(--border)', display:'flex', gap:10, justifyContent:'flex-end' }}>
              <Button variant="ghost" size="sm" onClick={() => setImportLinkModal(null)}>Skip</Button>
              <Button variant="primary" size="sm" loading={importLinkModal.saving} onClick={saveImportLink}>
                Save Document
              </Button>
            </div>
          </div>
