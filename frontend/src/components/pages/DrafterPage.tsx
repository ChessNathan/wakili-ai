...
import { Sparkles, RefreshCw, Copy, Printer, ChevronRight, FileText, ExternalLink, Upload, AlertCircle } from 'lucide-react';
...
export function DrafterPage() {
  ...
  async function exportToGoogle() {
    if (!doc) return;
    if (!googleStatus) {
      if (confirm('Google not connected. Go to Settings to connect your Google account?')) {
        navigate('/settings');
      }
      return;
    }
    ...
    try {
      ...
      setError(e.message || 'Failed to export to Google Docs');
    } finally {
              ...
              <div style={{padding:'13px 20px',borderBottom:'1px solid var(--border)',background:'var(--bg)',display:'flex',alignItems:'center',gap:10,flexWrap:'wrap'}}>
                ...
                {doc&&(
                  <div style={{display:'flex',gap:8}}>
                    ...
                    {!doc.google_doc_url && (
                      <Button size="sm" variant="secondary" leftIcon={<Upload size={13}/>} loading={exportingGoogle} onClick={exportToGoogle}
